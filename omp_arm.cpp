...
void C_GE_OMP_Static_neon(float** a, int n) {  // 使用neon进行SIMD优化的高斯消去算法，未对齐，使用fmsq
	float32x4_t va, vt, vaik, vakj, vaij;
 	float t1, t2;  // 使用两个浮点数暂存数据以减少程序中地址的访问次数
	int i, j, k;
#pragma omp parallel num_threads(THREAD_NUM) shared(a) private(i, j, k, t1, t2, va, vt, vaik, vakj, vaij)
...
void C_GE_OMP_Guided_neon(float** a, int n) {  // 使用neon进行SIMD优化的高斯消去算法，未对齐，使用fmsq
	float32x4_t va, vt, vaik, vakj, vaij;
 	float t1, t2;  // 使用两个浮点数暂存数据以减少程序中地址的访问次数
	int i, j, k;
#pragma omp parallel num_threads(THREAD_NUM) shared(a) private(i, j, k, t1, t2, va, vt, vaik, vakj, vaij)
...
void neon_GE(float** a, int n) {  // 使用neon进行SIMD优化的高斯消去算法，未对齐
...
void neon_GE_fms(float** a, int n) {  // 使用neon进行SIMD优化的高斯消去算法，未对齐，使用fmsq
	float32x4_t va, vt, vaik, vakj, vaij;
	for (int k = 0; k < n; k++)
	{
		vt = vmovq_n_f32(a[k][k]);  // 加载四个重复值到vt中
...
float** generate(int n) {
	ifstream inp("/home/s2012411/test/omp/input.txt");
	inp >> n;
	float** m = new float* [n];
	for (int i = 0; i < n; i++)
	{
...
int main() {
	...
	//-----------------------------------------------------------------
  	...
  	m2 = generate(n);
	gettimeofday(&start,NULL); //gettimeofday(&start,&tz);结果一样
	C_GE_OMP_Dynamic_neon(m2, n);
    gettimeofday(&end,NULL); //gettimeofday(&start,&tz);结果一样
    time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒
	cout << "C_GE_OMP_Dynamic_neon: " << time_use / 1000
		<< "ms" << endl;
	//show(m2, n);
	...
	//-----------------------------------------------------------------
  	...
  	m2 = generate(n);
	gettimeofday(&start,NULL); //gettimeofday(&start,&tz);结果一样
	C_GE_OMP_Static_neon(m2, n);
    gettimeofday(&end,NULL); //gettimeofday(&start,&tz);结果一样
    time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒
	cout << "C_GE_OMP_Static_neon: " << time_use / 1000
		<< "ms" << endl;
	//show(m2, n);
	...
	//-----------------------------------------------------------------
   	...
   	m2 = generate(n);
	gettimeofday(&start,NULL); //gettimeofday(&start,&tz);结果一样
	C_GE_OMP_Guided_neon(m2, n);
    gettimeofday(&end,NULL); //gettimeofday(&start,&tz);结果一样
    time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒
	cout << "C_GE_OMP_Guided_neon: " << time_use / 1000
		<< "ms" << endl;
	//show(m2, n);
    ...
    gettimeofday(&end,NULL); //gettimeofday(&start,&tz);结果一样
	time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//微秒
	cout << "neon_GE_fms: " << time_use / 1000
		<< "ms" << endl;
	//show(m4, n);
